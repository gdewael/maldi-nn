...
from maldi_nn.utils.data import SpeciesZSLDataModule
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Training script for zero-shot species identification.",
    ...
    parser.add_argument(
        ...
        type=boolean,
        default=True,
        help="ZSL mode (Dual branch NN with strain-lvl DNA side information) or not (regular multi-class clf on species-level).",
    )
    ...
    if not args.zsl_mode:

        species_to_genus_mapper = {
            int(species): int(genus)
            for species, genus in
            zip(dm.train.f["1/strain_to_species_ix"], dm.train.f["1/strain_to_genus_ix"])
        }

        genus_labels = torch.tensor([
            species_to_genus_mapper[i] for i in range(len(species_to_genus_mapper))
        ])
